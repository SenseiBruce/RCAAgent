...
class RcaIntegrationTest {
  ...
  @MockitoBean private LlmProvider llmProvider;
  ...
  void analyze_withLogContent_returnsStructuredResponse() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(
            jsonPath("$.rootCause")
                .value("NullPointerException in UserService.getUser() due to missing null check"))
  ...
  void analyze_emptyBody_returnsBadRequest() throws Exception {
    mockMvc
        .perform(post("/api/v1/rca/analyze").contentType(MediaType.APPLICATION_JSON).content("{}"))
