...
class RcaServiceTest {
  ...
  @Mock private LogAnalyzerService logAnalyzer;

  @Mock private GitAnalyzerService gitAnalyzer;

  @Mock private CodeContextService codeContext;

  @Mock private RepoResolver repoResolver;
  ...
  void analyze_withLogContent_callsLogAnalyzer() {
    RcaRequest request =
        new RcaRequest("NPE on login", null, "2024-01-15T10:30:00Z ERROR NPE", null, null, null);
    List<LogEntry> entries =
        List.of(new LogEntry(Instant.now(), "ERROR", "NPE", "", java.util.Map.of()));
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"null pointer\",\"severity\":\"HIGH\",\"evidenceFromLogs\":[\"NPE\"],\"recommendations\":[\"add null check\"]}");
  ...
  void analyze_withLogFilePath_callsAnalyzeFromFile() throws Exception {
    ...
    List<LogEntry> entries =
        List.of(new LogEntry(Instant.now(), "ERROR", "error", "", java.util.Map.of()));

    when(logAnalyzer.analyzeFromFile("/path/to/log")).thenReturn(entries);
    when(logAnalyzer.summarizeForLlm(any())).thenReturn("LOG SUMMARY");
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"file error\",\"severity\":\"MEDIUM\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
  ...
  void analyze_withRepoPath_callsGitAnalyzer() throws Exception {
    ...
    List<LogEntry> entries =
        List.of(new LogEntry(Instant.now(), "ERROR", "err", "", java.util.Map.of()));
    List<GitChange> commits =
        List.of(new GitChange("abc123", "dev", "fix", Instant.now(), List.of("App.java")));
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"bug\",\"severity\":\"HIGH\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
  ...
  void analyze_noRepoPath_skipsGitAnalysis() throws Exception {
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"test\",\"severity\":\"LOW\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
  ...
  void analyze_gitAnalyzerThrows_continuesGracefully() throws Exception {
    ...
    when(logAnalyzer.analyze(anyString())).thenReturn(entries);
    when(logAnalyzer.summarizeForLlm(any())).thenReturn("LOG");
    when(gitAnalyzer.getRecentCommits("/repo", "main"))
        .thenThrow(new RuntimeException("repo not found"));
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"test\",\"severity\":\"LOW\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
  ...
  void analyze_logAnalyzerThrows_continuesGracefully() throws Exception {
    ...
    when(logAnalyzer.analyzeFromFile("/bad/path"))
  ...
  void analyze_emptyLogContent_treatedAsNoLog() {
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"no logs\",\"severity\":\"LOW\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
  ...
  void analyze_emptyLogFilePath_usesLogContent() {
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn(
            "{\"rootCause\":\"from content\",\"severity\":\"MEDIUM\",\"evidenceFromLogs\":[],\"recommendations\":[]}");
    when(llmProvider.name()).thenReturn("test");
