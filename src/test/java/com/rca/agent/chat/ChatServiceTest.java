...
class ChatServiceTest {
  ...
  void chat_responseWithoutActionKeywords_treatedAsNormal() {
    when(llmProvider.analyze(anyString()))
        .thenReturn("I understand the issue. Let me ask a few questions.");
    ...
    when(rcaService.analyze(any()))
        .thenReturn(
            new RcaResponse(
                ...
                List.of(),
                List.of(),
                List.of("Increase pool size", "Add circuit breaker"),
                Instant.now()));
  ...
  void chat_fixAction_noPrCreated_returnsWarning() {
    ...
    when(autoFixService.fix(any(), anyString()))
        .thenReturn(new FixResponse(null, null, List.of(), "LLM could not generate a fix."));
  ...
  void chat_llmReturnsPartialActionJson_treatedAsNormal() {
    ...
    when(llmProvider.analyze(anyString()))
        .thenReturn("I'll take action: analyzing your params: now");
  ...
  void chat_llmReturnsMalformedJson_treatedAsNormal() {
    when(llmProvider.analyze(anyString()))
        .thenReturn("{\"action\": \"analyze\", \"params\": {broken json}}");
  ...
  void chat_llmThrows_propagatesException() {
    ...
    org.junit.jupiter.api.Assertions.assertThrows(
        RuntimeException.class, () -> chatService.chat(new ChatRequest("test", null)));
  }
}
