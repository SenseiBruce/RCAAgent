...
class ChatControllerTest {
  ...
  void chat_validRequest_returnsOk() throws Exception {
    when(chatService.chat(any())).thenReturn(ChatResponse.reply("How can I help?", "session-123"));
  ...
  void chat_withSessionId_returnsOk() throws Exception {
    when(chatService.chat(any())).thenReturn(ChatResponse.reply("I see.", "existing-session"));
  ...
  void chat_actionResponse_includesAction() throws Exception {
    when(chatService.chat(any()))
        .thenReturn(ChatResponse.withAction("Analysis complete!", "session-1", "rca_complete"));
