...
class SecurityTest {
  ...
  void analyze_sqlInjectionInDescription_returnsBadRequestOrHandled() throws Exception {
    when(rcaService.analyze(any()))
        .thenReturn(
            new RcaResponse(
                "safe", "LOW", List.of(), List.of(), List.of(), List.of(), Instant.now()));
  ...
  @ParameterizedTest
  @ValueSource(strings = {"../../../etc/passwd", "/etc/shadow", "/proc/self/environ"})
  void analyze_pathTraversalInLogFilePath_doesNotExpose(String maliciousPath) throws Exception {
    ...
    String requestJson =
        "{\"issueDescription\": \"test issue\", \"logFilePath\": \"" + maliciousPath + "\"}";
  ...
  void analyze_windowsPathTraversalInLogFilePath_doesNotExpose() throws Exception {
    ...
    String requestJson =
        "{\"issueDescription\": \"test\", \"logFilePath\": \"..\\\\..\\\\windows\\\\system32\\\\config\\\\sam\"}";
  ...
  void fix_headerInjection_doesNotPropagate() throws Exception {
    when(autoFixService.fix(any(), anyString()))
        .thenReturn(new FixResponse(null, null, List.of(), "ok"));
