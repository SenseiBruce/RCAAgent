 ...
 *
 * <p>Matches common log patterns with timestamps and severity levels using regex. Lines that don't
 * match the expected pattern are treated as INFO-level entries. This parser always returns {@code
 * true} for {@link #canParse(String)} as it serves as the fallback when no other parser matches.
 */
@Component
    ...
    for (String line : lines) {
      ...
      if (matcher.matches()) {
        ...
        try {
          timestamp =
              Instant.parse(
                  matcher.group(1).endsWith("Z") ? matcher.group(1) : matcher.group(1) + "Z");
        } catch (Exception e) {
