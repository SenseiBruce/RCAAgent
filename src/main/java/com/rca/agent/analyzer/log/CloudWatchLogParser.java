 ...
 *
 * <ul>
 *   <li>CloudWatch Logs API response — {@code {"events": [{"timestamp": epoch_ms, "message":
 *       "..."}]}}
 *   <li>CloudWatch Logs Insights results — lines with {@code @timestamp}, {@code @message},
 *       {@code @logStream}
 *   <li>Exported CloudWatch events array — {@code [{"timestamp": ..., "message": ...}]}
 * </ul>
 */
@Order(1)
@Component
public class CloudWatchLogParser implements LogParser {
  ...
  private static final Set<String> CW_MARKERS =
      Set.of("@timestamp", "@message", "@logStream", "@log");
  ...
  /**
   * Parses CloudWatch Logs Insights query results: {@code {"results": [[{"field": "@timestamp",
   * "value": "..."}, ...], ...]}}.
   */
  ...
  private LogEntry parseEvent(JsonNode event) {
    ...
    String logStream = event.path("logStreamName").asText(event.path("@logStream").asText(""));
  ...
  private LogEntry buildFromInsightsFields(Map<String, String> fields) {
    ...
    String level =
        fields.containsKey("@level") ? fields.get("@level").toUpperCase() : detectLevel(message);
