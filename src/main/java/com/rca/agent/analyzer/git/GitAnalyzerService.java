 ...
 *
 * <p>Provides capabilities for:
 *
 * <ul>
 *   <li>Retrieving recent commit history
 *   <li>File-level blame annotations
 *   <li>Commit diff generation
 *   <li>Summarizing git context for LLM consumption
 * </ul>
 */
@Service
public class GitAnalyzerService {
   ...
   * @throws Exception if the repository cannot be opened or read
   */
  public List<GitChange> getRecentCommits(String repoPath, String branch, TimeRange timeWindow)
      throws Exception {
    try (Git git = Git.open(new File(repoPath))) {
      ...
      Iterable<RevCommit> commits =
          git.log().add(git.getRepository().resolve(targetBranch)).setMaxCount(maxCommits).call();
  ...
  private List<String> getChangedFiles(Repository repository, RevCommit commit) {
    try {
      ...
      try (ObjectReader reader = repository.newObjectReader();
        ...
        return formatter.scan(oldTree, newTree).stream().map(DiffEntry::getNewPath).toList();
      }
    } catch (IOException e) {
