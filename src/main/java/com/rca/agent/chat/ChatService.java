...
public class ChatService {
  ...
  private List<String> suggestQuickReplies(String llmResponse, List<ChatMessage> history) {
    ...
    if (lower.contains("log")
        && (lower.contains("share") || lower.contains("paste") || lower.contains("provide"))) {
  ...
  private ChatResponse checkForAction(
      String llmResponse, String sessionId, List<ChatMessage> history) {
    try {
  ...
  private String formatFixResult(FixResponse response) {
    if (response.pullRequestUrl() != null) {
      return "## ✅ Auto-Fix PR Created!\n\n**PR:** "
          ...
          + response.branchName()
          + "\n**Files:** "
          + String.join(", ", response.filesChanged())
          + "\n\n"
          + response.summary();
    }
