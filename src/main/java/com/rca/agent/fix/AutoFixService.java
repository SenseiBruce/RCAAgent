 ...
 *
 * <ol>
 *   <li>Ask LLM to generate code fix + unit tests based on root cause
 *   <li>Apply changes to a new branch in the cloned repo
 *   <li>Push the branch
 *   <li>Create a pull/merge request via the appropriate platform (GitHub/GitLab)
 * </ol>
 */
@Service
public class AutoFixService {
  ...
  public AutoFixService(
      LlmProvider llmProvider, RepoResolver repoResolver, List<GitPlatform> platforms) {
  ...
  public FixResponse fix(FixRequest request, String token) {
    ...
    try {
      ...
      if (changes.isEmpty()) {
        log.warn(
            "No valid file changes parsed from LLM response. First 200 chars: {}",
  ...
  private String createPrOnPlatform(
      String repoUrl, String headBranch, String baseBranch, FixRequest request, String token) {
  ...
  private String buildFixPrompt(FixRequest request) {
    ...
    if (request.codeSnippets() != null && !request.codeSnippets().isEmpty()) {
      sb.append(
          "CURRENT CODE AT ERROR LOCATIONS (you MUST use these exact file paths in your response):\n");
      request
          .codeSnippets()
          .forEach(
              s ->
    ...
    try {
      ...
      for (JsonNode change : changesNode) {
        String filePath =
            change
                .path("filePath")
                .asText(change.path("file_path").asText(change.path("path").asText("")));
        ...
        if (!searchReplaceNode.isMissingNode() && searchReplaceNode.isArray()) {
          ...
          if (!search.isBlank()) {
            changes.add(
                new FileChange(filePath, null, List.of(new SearchReplace(search, replace))));
          }
        } else {
          // Legacy: full content mode
          String content = change.path("content").asText(change.path("code").asText(""));
          if (!content.isBlank()) {
  ...
  private void applyAndPush(
      Path repoPath, String branchName, List<FileChange> changes, FixRequest request, String token)
      throws Exception {
    try (Git git = Git.open(repoPath.toFile())) {
          ...
          for (SearchReplace patch : change.patches()) {
            if (modified.contains(patch.search())) {
              ...
              log.warn(
                  ...
                  change.filePath(),
                  patch.search().substring(0, Math.min(50, patch.search().length())));
            }
          }
      ...
      if (filesModified == 0) {
        throw new IllegalStateException(
            "No files were modified — fix could not be applied to this repo");
      }

      git.commit().setMessage("fix: auto-fix for - " + truncate(request.rootCause(), 72)).call();
  ...
  /**
   * Resolves file paths that the LLM returned as bare filenames (e.g. "RcaService.java") to their
   * actual relative path in the repo (e.g. "src/main/java/com/rca/agent/service/RcaService.java").
   */
