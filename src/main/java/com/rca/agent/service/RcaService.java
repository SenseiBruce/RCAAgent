 ...
 *
 * <p>Coordinates log analysis, source code extraction, git history analysis, and LLM-based
 * reasoning to produce a structured root cause determination. Supports both local repo paths and
 * remote git URLs.
 */
@Service
public class RcaService {
  ...
  public RcaService(
    ...
    this.analysisTimer =
        Timer.builder("rca.analysis.duration")
            .description("Time taken for RCA analysis")
            .register(meterRegistry);
    this.analysisCounter =
        Counter.builder("rca.analysis.total")
            .description("Total RCA analyses performed")
            .register(meterRegistry);
    this.analysisErrorCounter =
        Counter.builder("rca.analysis.errors")
  ...
  private RcaResponse doAnalyze(RcaRequest request) {
    ...
    try {
      ...
      String logSummary =
          logEntries.isEmpty() ? "No log data provided." : logAnalyzer.summarizeForLlm(logEntries);

      // Step 2: Extract file references once, reuse for code + git
      List<FileReference> fileRefs =
          logEntries.isEmpty() ? List.of() : codeContext.extractFileReferences(logEntries);
  ...
  private String analyzeGit(
      String repoPath, String branch, List<FileReference> refs, TimeRange timeWindow) {
  ...
  private TimeRange resolveTimeWindow(String timeWindowSpec, List<LogEntry> logEntries) {
    ...
    if (logEntries.size() >= 2) {
      List<Instant> timestamps = logEntries.stream().map(LogEntry::timestamp).sorted().toList();
  ...
  private RcaResponse parseResponse(
      String llmResponse, List<CodeSnippet> codeSnippets, List<GitChange> commits) {
    try {
      ...
      return new RcaResponse(
          ...
          node.path("evidenceFromLogs").findValuesAsText("evidenceFromLogs").isEmpty()
              ? extractArray(node, "evidenceFromLogs")
              : List.of(),
          codeSnippets,
          commits,
      ...
      return new RcaResponse(
          llmResponse, "MEDIUM", List.of(), codeSnippets, commits, List.of(), Instant.now());
    }
  }
